...
"""
整合修改器 - 简单字符串匹配版
使用简单字符串匹配的BDDL修改器，同时处理init_state数组
"""

from typing import Dict, List, Tuple, Optional
import os
from bddl_modifier_simple_match import BDDLModifierSimpleMatch
from init_state_modifier_simple import InitStateModifierSimple, create_sample_init_state, save_list_to_json


class IntegratedModifierSimpleMatch:
    def __init__(self):
        self.bddl_modifier = BDDLModifierSimpleMatch()
    ...
    def process_bddl_and_init_state(self, 
        ...
        """
        同时处理BDDL文件和init_state数组（使用简单字符串匹配）
        ...
        Returns:
            Dict: 包含所有修改结果的字典
        """
        print("=== 开始处理BDDL文件和init_state数组（简单字符串匹配版）===\n")
        ...
        if output_init_state:
            ...
            output_init_state = f"{base_name}_simple_match_modified_init_state.json"
            save_list_to_json(modified_init_state, output_init_state)
            print(f"4. 保存修改后的init_state到: {output_init_state}")
        ...
        integrated_result = {
            ...
            'summary': {
                ...
                'matching_method': '简单字符串匹配'
            }
        }
        
        return integrated_result
    
    def generate_integrated_report(self, result: Dict, report_file: str = "integrated_simple_match_report.md"):
        """生成简单字符串匹配版整合报告"""
        report_content = f"""# 整合修改报告（简单字符串匹配版）
...
- 匹配方法: {result['summary']['matching_method']}
...
"""
        ...
        print(f"✅ 简单字符串匹配版整合报告已保存到: {os.path.abspath(report_file)}")
        return os.path.abspath(report_file)


def example_simple_match_usage():
    """简单字符串匹配版整合使用示例"""
    print("=== 简单字符串匹配版整合修改器使用示例 ===\n")
    ...
    # 2. 创建简单字符串匹配版整合修改器并处理
    integrated_modifier = IntegratedModifierSimpleMatch()
    
    try:
        result = integrated_modifier.process_bddl_and_init_state(
            bddl_file=bddl_file,
            init_state=init_state,
            output_bddl="problem_flat_stove_simple_match_integrated_modified.bddl",
            output_init_state="init_state_flat_stove_simple_match_modified.json",
            report_file="bddl_flat_stove_simple_match_report.md"
        )
        
        print("\n" + "="*60)
        print("✅ 简单字符串匹配版整合处理完成！")
        print("\n📊 处理结果统计:")
        ...
        integrated_report = integrated_modifier.generate_integrated_report(
            result, "complete_simple_match_integrated_report.md"
        )
        ...
        # 显示关键信息
        print(f"\n📋 匹配结果:")
        print(f"  匹配方法: 检查物体名称是否包含在'{result['obj_of_interest']}'中")
        print(f"  删除的objects位置: {result['deleted_objects_positions']}")
        
        # 警告信息
        if len(result['deleted_objects_positions']) > 0:
            print(f"\n⚠️  重要提醒:")
            print(f"  请检查BDDL文件的:goal字段是否仍然引用已删除的objects")
            print(f"  如果有引用，可能需要手动调整:goal字段")
...
if __name__ == "__main__":
    # 运行简单字符串匹配版整合示例
    result = example_simple_match_usage()
    
    if result:
        print(f"\n🎉 简单字符串匹配版整合修改完成！")
        print(f"✅ 使用严格的字符串匹配规则完成了BDDL文件和init_state数组的同步修改")
    else:
        print(f"\n💥 简单字符串匹配版整合修改失败，请检查错误信息。")