...
from init_state_modifier import InitStateModifier


class IntegratedModifier:
    def __init__(self):
        self.bddl_modifier = BDDLModifier()
        self.init_state_modifier = InitStateModifier()
    ...
    def process_bddl_and_init_state(self, 
        ...
        if output_init_state:
            np.save(output_init_state, modified_init_state)
            print(f"4. 保存修改后的init_state到: {output_init_state}")
        else:
            # 默认保存路径
            base_name = os.path.splitext(bddl_file)[0]
            output_init_state = f"{base_name}_modified_init_state.npy"
            np.save(output_init_state, modified_init_state)
            print(f"4. 保存修改后的init_state到: {output_init_state}")
...
2. 加载修改后的init_state数组:
   ```python
   import numpy as np
   modified_init_state = np.load('{os.path.basename(result['modified_init_state_file'])}')
   ```
...
def example_integrated_usage():
    ...
    # 创建示例数据
    from init_state_modifier import create_sample_init_state
    ...
    print(f"  init_state形状: {init_state.shape}")
    print()
    
    # 2. 创建整合修改器并处理
    integrated_modifier = IntegratedModifier()
    
    try:
        result = integrated_modifier.process_bddl_and_init_state(
            bddl_file=bddl_file,
            init_state=init_state,
            output_bddl="problem_integrated_modified.bddl",
            output_init_state="init_state_integrated_modified.npy",
            report_file="bddl_integrated_report.md"
        )
        ...
        integrated_report = integrated_modifier.generate_integrated_report(
            result, "complete_integrated_report.md"
        )
        ...
        print(f"  修改后init_state形状: {result['modified_init_state_array'].shape}")
        ...
        # 验证数据一致性
        expected_columns = init_state.shape[1] - len(result['deleted_objects_positions']) * 7
        actual_columns = result['modified_init_state_array'].shape[1]
